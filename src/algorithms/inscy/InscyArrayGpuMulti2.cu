...
void
InscyArrayGpuMemory(int *d_neighborhoods, int *d_neighborhood_end, TmpMalloc *tmps, ScyTreeArray *scy_tree,
                    float *d_X, int n, int d, float neighborhood_size, float F,
                    int num_obj,
                    int min_size, map<vector<int>, int *, vec_cmp> &result, int first_dim_no,
                    int total_number_of_dim, float r, int &calls, bool rectangular) {
    calls++;
    int total_inscy = pow(2, d);
    printf("InscyArrayGpuMemory(%d): %d%%      \r", calls, int((calls * 100) / total_inscy));
    ...
    while (start_dim_no < total_number_of_dim) {
        ...
        vector <vector<ScyTreeArray *>> L_merged = scy_tree->restrict_merge_gpu_multi5(tmps, start_dim_no,
        ...
        while (dim_no < end_dim_size) {
            ...
            for (ScyTreeArray *restricted_scy_tree : L_merged[i]) {
                ...
                if (pruneRecursion) {
                    InscyArrayGpuMemory(d_new_neighborhoods, d_new_neighborhood_end, tmps, restricted_scy_tree,
                                        d_X, n, d, neighborhood_size, F, num_obj, min_size,
                                        result, dim_no + 1, total_number_of_dim, r, calls, rectangular);
...
void
InscyArrayGpuMemory2(int *&d_neighborhoods, int *&d_neighborhood_end, TmpMalloc *tmps, ScyTreeArray *scy_tree,
                     float *d_X, int n, int d, float neighborhood_size, float F,
                     int num_obj,
                     int min_size, map<vector<int>, int *, vec_cmp> &result, int first_dim_no,
                     int total_number_of_dim, float r, int &calls, bool rectangular) {
    calls++;
    int total_inscy = pow(2, d);
    printf("InscyArrayGpuMemory2(%d): %d%%      \r", calls, int((calls * 100) / total_inscy));


    int total_size = 1;
    printf("\n1\n");
    if (scy_tree->number_of_restricted_dims>0) {
        cudaMemcpy(&total_size, d_neighborhood_end + n - 1, sizeof(int), cudaMemcpyDeviceToHost);
    }
    printf("\n2\n");
    int *h_neighborhoods;
    if(total_size>0) {
    ...
    printf("\n2.1 %d\n", total_size);
    if (scy_tree->number_of_restricted_dims>0 && total_size>0) {
        cudaMemcpy(h_neighborhoods, d_neighborhoods, total_size * sizeof(int), cudaMemcpyDeviceToHost);
    }
    printf("\n2.2\n");
    ...
    while (start_dim_no < total_number_of_dim) {
        ...
        while (dim_no < end_dim_size) {
            ...
            for (ScyTreeArray *restricted_scy_tree : L_merged[i]) {
                ...
                if (pruneRecursion) {
                    InscyArrayGpuMemory2(d_new_neighborhoods, d_new_neighborhood_end, tmps, restricted_scy_tree,
                                         d_X, n, d, neighborhood_size, F, num_obj, min_size,
                                         result, dim_no + 1, total_number_of_dim, r, calls, rectangular);
        ...
        if (scy_tree->number_of_restricted_dims>0 && total_size>0) {
            ...
            cudaMemcpy(d_neighborhoods, h_neighborhoods, total_size * sizeof(int), cudaMemcpyHostToDevice);
        }
    }
}
...
void
InscyArrayGpuStar(int *d_neighborhoods, int *d_neighborhood_end, TmpMalloc *tmps, ScyTreeArray *scy_tree,
                  float *d_X, int n, int d, float neighborhood_size, float F,
                  int num_obj,
                  int min_size, map<vector<int>, int *, vec_cmp> &result, int first_dim_no,
                  int total_number_of_dim, float r, int &calls, bool rectangular) {
    calls++;
    int total_inscy = pow(2, d);
    printf("InscyArrayGpuStar(%d): %d%%      \r", calls, int((calls * 100) / total_inscy));
    ...
    pair<int **, int **> p = find_neighborhoods_re_star(tmps, d_neighborhoods, d_neighborhood_end,
                                                        d_X, n, d, scy_tree, L_merged, neighborhood_size);
    ...
    while (dim_no < total_number_of_dim) {
        ...
        for (ScyTreeArray *restricted_scy_tree : L_merged[i]) {
            ...
            if (pruneRecursion) {
                ...
                InscyArrayGpuStar(d_new_neighborhoods, d_new_neighborhood_end, tmps, restricted_scy_tree,
                                  d_X, n, d, neighborhood_size, F, num_obj, min_size,
                                  result, dim_no + 1, total_number_of_dim, r, calls, rectangular);
