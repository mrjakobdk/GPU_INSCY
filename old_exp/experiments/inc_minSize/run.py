...
minSizes = [15, 45, 75, 105, 150]

times = []
no_clusters = []
subspaces, clusterings = INSCY.run_gpu(X[:100, :2], params["neighborhood_size"], 1., 8, 4)
for minSize in minSizes:
    print("minSize:", minSize)
    t0 = time.time()
    for _ in range(1):
        subspaces, clusterings = function(X, params["neighborhood_size"], params["F"],
                                          params["num_obj"], minSize, r=params["r"],
    ...
    print("clusters:",no_clusters[-1])
    print()
    np.savez('plot_data/inc_minSize/' + name + '.npz', minSizes=minSizes, no_clusters=no_clusters, times=times, params=params)
