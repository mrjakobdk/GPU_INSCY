...
for n in ns:
    ...
    t0 = time.time()
    for _ in range(5):
        subspaces, clusterings = INSCY.run_gpu_multi2_cl_multi_mem(X_, params["neighborhood_size"], params["F"],
                                               params["num_obj"], max(1, int(n * params["min_size"])))
    times.append((time.time() - t0)/5)
    print("Finished INSCY, took: %.4fs" % (time.time() - t0))
    print()

    np.savez('plot_data/inc_n/multi2_cl_multi_mem.npz', ns=ns, times=times, params=params)