...
for subspace_size in subspace_sizes:
    ...
    t0 = time.time()
    for _ in range(5):
        subspaces, clusterings = INSCY.run_gpu_multi2_cl_multi_mem(X_, params["neighborhood_size"], params["F"], params["num_obj"], params["min_size"],5)
    times.append((time.time() - t0)/5)
    print("Finished INSCY, took: %.4fs" % (time.time() - t0))
    print()

np.savez('plot_data/inc_d/const_n_gpu_multi2_cl_multi_mem.npz', subspace_sizes=subspace_sizes, times=times, params=params)