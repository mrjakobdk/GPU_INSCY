...
times = []
subspaces, clusterings = INSCY.run_gpu_multi(X[:, :2], params["neighborhood_size"], params["F"], params["num_obj"], params["min_size"])
for subspace_size in subspace_sizes:
    X_ = X[:, :subspace_size].clone()
    print("d:",subspace_size)
    t0 = time.time()
    subspaces, clusterings = INSCY.run_gpu_multi(X_, params["neighborhood_size"], params["F"], params["num_obj"], params["min_size"])
    times.append(time.time() - t0)
    print("Finished INSCY, took: %.4fs" % (time.time() - t0))
    print()

np.savez('plot_data/inc_d/const_n=300_gpu_multi.npz', subspace_sizes=subspace_sizes, times=times, params=params)